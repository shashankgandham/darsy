(function() {
	...
	curve.prototype.equation = function(i) {
		var p = this.tick;
		var x = this.controller.amplitude * this.amplitude * this.controller.MAX;
		var y = -1 * Math.abs(Math.sin(p)) * x * Math.pow(1/(1+Math.pow(this.open_class*i,2)),2);
		if (Math.abs(y) < 0.001) {
			this.respawn();
		}
		return y;
	...
	function wave(opt) {
		...
		this.curves = [];
		for (var i = 0; i < wave.prototype.COLORS.length; i++) {
			var color = wave.prototype.COLORS[i];
	...
	wave.prototype._interpolate = function(propertyStr) {
	...
	wave.prototype._startDrawCycle = function() {
	...
	wave.prototype.setSpeed = function(v, increment) {
		this._interpolation.speed = v;
	};

	wave.prototype.setNoise = wave.prototype.setAmplitude = function(v) {
		this._interpolation.amplitude = Math.max(Math.min(v, 1), 0);
	};

	wave.prototype.COLORS = [
		[32,133,252],
		[94,252,169],
		[253,71,103]
	];
